...
class Instance:
    ...
    def score(self) -> int:
        return self._employee_violences() + self.__shift_cover_violences()

    def __shift_cover_violences(self) -> int:
        return sum([__shift_cover_violence(shift_cover) for shift_cover in self.shift_covers])

    def __shift_cover_violence(self, shift_cover: ShiftCover) -> int:
        cover = sum([(1 if shift_cover.day in employee.shift_assings else 0)
    ...
    def __employee_violences(self) -> int:
...
class Employee:
    ...
    def __off_request_violence(self, assign: ShiftAssign) -> int:
        return self.off_requests[assign.day].weight if assign.day in self.off_requests else 0

    def on_request_violences(self) -> int:
        return sum([self.__on_request_violence(request) for request in self.on_requests])

    def __on_request_violence(self, request: OnRequest) -> int:
